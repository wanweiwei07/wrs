...
if __name__ == '__main__':
    ...
    # path = rrtc.plan(seed_jnt_vals=np.array([0, 0]), end_conf=np.array([5, 10]), obstacle_list=obstacle_list,
    ...
    for i in range(100):
        ...
        path = rrtc.plan(start_conf=np.array([0, 0]), goal_conf=np.array([5, 10]), obstacle_list=obstacle_list,
                         ext_dist=1, rand_rate=70, max_time=300, component_name='all', animation=False)
        toc = time.time()
