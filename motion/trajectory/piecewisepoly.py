...
class PiecewisePoly(object):
    ...
    def _linear_interpolate(self, A, samples):
        ...
        return interpolated_y, interpolated_y_dot, interpolated_y_dotdot
    ...
    def _cubic_interpolate(self, A, samples):
        ...
        for i in range(n_sections):
            if i == n_sections-1: # last
                interpolated_x_sec = np.array((samples+i).tolist())
            else:
                interpolated_x_sec = np.array((samples+i).tolist()[:-1])
            # y
    ...
    def interpolate(self, path, control_frequency=.005, time_interval=1.0):
        ...
        A = self._solve()
        return self._interpolate(A, samples)