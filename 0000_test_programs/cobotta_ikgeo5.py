...
prev_error = rm.nan
tic = time.time()
for i, q4 in enumerate(rm.np.linspace(arm.jlc.jnts[3].motion_range[0], arm.jlc.jnts[3].motion_range[1], 360)):
    e_q4_dict[i]=[]
...
for cand in candidate_jnt_values:
    print(rm.np.degrees(cand))
print("---------------------")
new_result = []
