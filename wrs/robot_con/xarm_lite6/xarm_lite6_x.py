...
class XArmLite6X(object):
    ...
    def get_gripper_width(self) -> float:
        if not self._is_gripper_calibrated or self._zero_dxl_pos is None:
            raise Exception("Calibrate grippers first")
    ...
    def manual_calibrate_gripper(self):
        """
        Manually calibrate the grippers. Only run this functions when the gear and racks on the grippers are separated.
        Process:
