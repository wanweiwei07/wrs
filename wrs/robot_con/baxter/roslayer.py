...
# preinstallation
# basxter_interface https://github.com/RethinkRobotics/baxter_interface
# ADD dist-packages to python
sys.path.append("/opt/ros/indigo/lib/python2.7/dist-packages")
# Settup the Environment variables
os.environ["ROS_ROOT"] = "/opt/ros/indigo/share/ros"
os.environ["ROS_PACKAGE_PATH"] = "/opt/ros/indigo/share:/opt/ros/indigo/stacks"
...
class Baxter:
    ...
    def movejnts(self,jnts_dict, speed =1 , armname="rgt"):
        # if speed < 0 or speed >1: raise Exception("Speed Error")
        if not self.__enabled: return
        limb = self.__right_limb if armname == "rgt" else self.__left_limb
        ...
        anglesbetween = np.array(path_angle) - np.array(currentjnts)
        anglesbetween = np.abs(anglesbetween)
        anglesbetween[:3] /= (np.pi / 4)
        anglesbetween[3:] /= (np.pi / 3)
        time = np.max(anglesbetween)
        # for i in range(len(currentjnts)):
        #
        #     time = max(abs(path_angle[i]-currentjnts[i])/ (3.1415926/4),time)
        rospy.loginfo("The time of path: %f" % time)
        traj.add_point(path_angle, time)
        traj.start()
        traj.wait(-1)
        traj.clear(limb.name)
    ...
    def movejnts_cont(self, jnts_dict_list, speed=1, armname="rgt"):
        ...
        t = speed
        path_angle_list = []
        for index,jnts_dict in enumerate(jnts_dict_list):
            # limb.move_to_joint_positions(jnts_dict, threshold= baxter_interface.settings.JOINT_ANGLE_TOLERANCE)
            ...
            if isinstance(jnts_dict,dict):
                ...
                path_angle = jnts_dict
            path_angle_list.append(path_angle)
            # print(path_angle)
        anglesbetween =  np.array(path_angle_list) - np.vstack([currentjnts,  np.array(path_angle_list)])[:-1]
        anglesbetween = np.abs(anglesbetween)
        anglesbetween[:,3] /= (np.pi / 4)
        anglesbetween[:,3:] /= (np.pi / 3)
        for index, jnts_dict in enumerate(anglesbetween):
            time = np.max(jnts_dict)
            print(time)
            t += time
            traj.add_point(path_angle_list[index],t)
        traj.start()
        traj.wait(-1)
        limb.set_joint_position_speed(.3)
        traj.clear(limb.name)
...
if __name__ == "__main__":
    b = Baxter(usegripper=False)
    # b.opengripper("rgt")
    # b.opengripper("lft")
    # angles = b.getjnts("rgt")
    #
    # print b.getforce()
    # print b.getjnts()
    # print b.currentposgripper()
    # cameras: ['head_camera', 'left_hand_camera', 'right_hand_camera']

    cv.imshow(b.getimage("left_hand_camera"))
    cv.waitKey(0)

    # angles = b.getjnts(armname="rgt")
    # angles["right_w0"] += 0.3
    # angles["right_w1"] += 0.2
    # angles["right_w2"] -= 0.4
    # print (angles)
    # b.movejnts(angles,speed=1,armname="rgt")
    # a.close()
