...
def add_root_log_file(log_file):
    """
    Add a log file to the path logger.
    ...
    # add a file handle to the path logger
...
class Logger(object):
    ...
    def reconfigure_root():
        """Reconfigure the path logger."""
        configure_root()

    @staticmethod
    def get_logger(name, log_level=logging.INFO, log_file=None, global_log_file=False, silence=False):
        """
        Build a logger. All logs will be propagated up to the path logger if not silenced. If log_file is provided, logs will be written out to that file. If global_log_file is true, log_file will be handed the path logger, otherwise it will only be used by this particular logger.
        ...
        global_log_file :obj:`bool`
            Whether or not to use the given log_file for this particular logger or for the path logger.
        silence :obj:`bool`
        ...
        # configure the path logger if it hasn't been already
        ...
        # silence the logger by preventing it from propagating upwards to the path
        logger.propagate = not silence

        # configure the log file stream 
        if log_file is not None:
            # if the log file is global, add it to the path logger
            if global_log_file:
    ...
    def add_log_file(logger, log_file, global_log_file=False):
        """
        Add a log file to this logger. If global_log_file is true, log_file will be handed the path logger, otherwise it will only be used by this particular logger.
