                    ...
                    if is_add:
                        ...
                        if cur_pos >= last_pos:
                            ...
                            if count2 >= 10:
                                break
                    if count >= 7:
                        # print('grippers target: {}, current: {}'.format(pos, cur_pos))
                        break
                    time.sleep(0.2)
                else:
                    return _
            # print('grippers, pos: {}, last: {}'.format(pos, last_pos))
            return ret[0]
        else:
            return ret[0]
    ...
    def _get_gripper_err_code(self):
        ...
        if self._check_code(ret[0]) == 0:
            ...
            if ret[0] == XCONF.UxbusState.ERR_CODE:
                self.get_err_warn_code()
                if self.error_code == 19 or self.error_code == 28:
                    print('grippers/tgpio error, code=C{}, G{}'.format(self.error_code, ret[1]))
                    return ret[0], ret[1]
            ret[0] = 0
            return ret[0], ret[1]
        return ret[0], 0
