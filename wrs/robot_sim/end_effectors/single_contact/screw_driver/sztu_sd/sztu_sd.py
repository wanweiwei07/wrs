...
class SZTU_SD(si.SCTInterface):
    ...
    def __init__(self,
        ...
        self.jlc = rkjlc.JLChain(pos=self.coupling.gl_flange_pose_list[0][0],
                                 rotmat=self.coupling.gl_flange_pose_list[0][1], n_dof=0, name='sztu_sd_jlc')
        self.jlc.anchor.lnk_list[0].name="sztu_sd_screwdriver"
        self.jlc.anchor.lnk_list[0].loc_rotmat=rm.rotmat_from_euler(-rm.pi/2,0,0)
        self.jlc.anchor.lnk_list[0].cmodel = mcm.CollisionModel(
            initor=os.path.join(current_file_dir, "meshes", "screwdriver_new.stl"), name="sztu_screwdriver",
            cdmesh_type=self.cdmesh_type)
        self.jlc.anchor.lnk_list[0].cmodel.rgba = rm.np.array([.9, .77, .52, 1.0])
        # reinitialize
        self.jlc.finalize()
        #  action center
        self.loc_acting_center_pos = self.coupling.loc_flange_pose_list[0][1] @ rm.np.array([0, 0, 0.059]) + self.coupling.loc_flange_pose_list[0][0]
