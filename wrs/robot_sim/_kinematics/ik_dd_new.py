...
class DDIKSolver(object):
    ...
    def _build_data(self):
        ...
        for id in tqdm(range(len(sampled_qs))):
            ...
            jmat_pinv = np.linalg.pinv(j_mat, rcond=1e-4)
            # relative to base
            ...
            jnt_data.append([jnt_values, np.concatenate((rel_pos, rel_rotvec)), jmat_pinv])
    ...
    def persist_data(self):
        ...
        with open(self._fname_jnt, 'wb') as f_jnt:
            pickle.dump(np.array(self.jnt_data, dtype=object), f_jnt)
    ...
    def ik(self,
        ...
        if seed_jnt_values is not None:
            ...
            candidate = self.jnt_data[np.array(nn_indx_list, dtype=int)]
            seed_posrot_array = np.stack(candidate[:, 1])
            seed_posrot_diff_array = query_point-seed_posrot_array
            square_sums = np.sum(seed_posrot_diff_array ** 2, axis=1)
            sorted_indices = np.argsort(square_sums)
            # seed_jmat_pinv_array = np.stack(candidate[:, 2])
            # adjust_array = np.einsum('ijk,ik->ij', seed_jmat_pinv_array, seed_posrot_diff_array)
            # square_sums = np.sum((np.stack(candidate[:, 0])+adjust_array) ** 2, axis=1)
            # sorted_indices = np.argsort(square_sums)
            for id, nn_indx in enumerate(sorted_indices[:2]):
                seed_jnt_values, seed_posrot, seed_jmat_pinv = self.jnt_data[nn_indx]
                if toggle_dbg:
