...
def parse_origin(node):
    ...
    :param node: :class`lxml.etree.Element` : An XML node which (optionally) has a child node with the ``origin`` tag.
    :return: (4,4) homomat, defaults to identity matrix if no ``origin`` child was found.
    """
    homomat = np.eye(4, dtype=np.float64)
    origin_node = node.find('origin')
    if origin_node is not None:
        if 'xyz' in origin_node.attrib:
            homomat[:3, 3] = np.fromstring(origin_node.attrib['xyz'], sep=' ')
        if 'rpy' in origin_node.attrib:
            rpy = np.fromstring(origin_node.attrib['rpy'], sep=' ')
            homomat[:3, :3] = rm.rotmat_from_euler(*rpy)
    return homomat


def unparse_origin(homomat):
    """
    Turn a 4x4 homogenous matrix into an ``origin`` XML node.
    :param homomat: (4,4) float : The 4x4 homogneous transform matrix to convert into an ``origin`` XML node.
    :return: :class`lxml.etree.Element` : An XML node whose tag is ``origin``. The node has two attributes:
        - ``xyz`` - A string with three space-delimited floats representing the translation of the origin.
        - ``rpy`` - A string with three space-delimited floats representing the rotation of the origin.
    """
    node = ET.Element('origin')
    node.attrib['xyz'] = '{} {} {}'.format(*homomat[:3, 3])
...
def get_filename(base_path, file_path, makedirs=False):
    ...
    :param base_path: The base path to the URDF's folder.
    :param file_path: The path to the file.
    :param makedirs: If ``True``, the directories leading to the file will be created if needed.
    :return: The resolved filepath -- just the normal ``file_path`` if it was an absolute path, otherwise that path
            joined to ``base_path``.
    """
    fn = file_path
...
def load_meshes(filename):
    ...
    :return: list of :class:`~trimesh.base.Trimesh` The meshes loaded from the file.
    """
    ...
    if isinstance(meshes, (list, tuple, set)):
        ...
        for r in meshes:
            if not isinstance(r, trm.Trimesh):
                raise TypeError('Could not load meshes from file')
    elif isinstance(meshes, trm.Trimesh):
        meshes = [meshes]
    else:
...
def configure_origin(value):
    ...
    :param value: None, (6,) float, or (4,4) float, The value to turn into the matrix. if (6,), interpreted as xyzrpy
    :return: homomat
    """
    if value is None:
...
class URDFType(object):
    ...
    def __init__(self):
        """
        Abstract base class for all URDF types. This has useful class methods for automatic parsing/unparsing
        of XML trees. There are three overridable class variables:
        - ``_ATTRIBS`` - This is a dictionary mapping attribute names to a tuple, ``(type, required)`` where ``type`` is
            the Python type for the attribute and ``required`` is a boolean stating whether the attribute is required to
            be present.
        - ``_ELEMENTS`` - This is a dictionary mapping element names to a tuple, ``(type, required, multiple)`` where
            ``type`` is the Python type for the element, ``required`` is a boolean stating whether the element is
             required to be present, and ``multiple`` is a boolean indicating whether multiple elements of this type
             could be present. Elements are child nodes in the XML tree, and their type must be a subclass of
             :class:`.URDFType`.
    ...
    def _parse_attrib(cls, val_type, val):
        ...
        :param val_type: The type of value to create.
    ...
    def _parse_simple_attribs(cls, node):
        ...
        :param node: The node to parse attributes for.
        :return: dict Map from attribute name to value. If the attribute is not required and is not present, that
        attribute's name will map to `None``.
        """
        kwargs = {}
    ...
    def _parse_simple_elements(cls, node, path):
        """
        Parse all elements in the _ELEMENTS array from the children of this node.
        :param node: The node to parse children for.
        :param path: The string path where the XML file is located (used for the location of mesh or image files).
        :return: dict Map from element names to the :class:`URDFType` subclass (or list, if ``multiple`` was set)
    ...
    def _parse(cls, node, path):
        """
        Parse all elements and attributes in the _ELEMENTS and _ATTRIBS arrays for a node.
        :param node: The node to parse.
        :param path: The string path where the XML file is located
        :return: Map (dict) from names to Python classes created from the attributes and elements in the class arrays.
        """
    ...
    def _from_xml(cls, node, path):
        ...
        :param node: The node to parse.
        :param path: The string path where the XML file is located
        :return: An instance of this class parsed from the node.
        """
    ...
    def _unparse_attrib(self, val_type, val):
        ...
        :param val_type: The type of the Python object.
    ...
    def _unparse_simple_attribs(self, node):
        """
        Convert all Python types from the _ATTRIBS array back into attributes for an XML node.
        :param node: The XML node to add the attributes to.
        :return:
        """
    ...
    def _unparse_simple_elements(self, node, path):
        """
        Unparse all Python types from the _ELEMENTS array back into child nodes of an XML node.
        :param node: The XML node for this object. Elements will be added as children of this node.
        :param path: The string path where the XML file is being written to (used for writing out meshes and image files).
        :return:
        """
    ...
    def _unparse(self, path):
        """
        Create an XML node for this object and unparse all elements and attributes in the class arrays.
        :param path: The string path where the XML file is being written to (used for writing out meshes and image files).
    ...
    def _to_xml(self, parent, path):
        ...
        :param parent: The parent node that this element will eventually be added to. This base implementation doesn't
                        use this information, but classes that override this function may use it.
        :param path: The string path where the XML file is being written to
        :return: The newly-created node.
        """
...
class Box(URDFType):
    ...
    def __init__(self, size):
        ...
        :param size: (3,) float : The length, width, and height of the box in meters.
        """
        self.size = size
        self._meshes = []
    ...
    def copy(self, prefix='', scale=None):
        ...
        :param prefix: A prefix to apply to all names.
        :param scale:
        :return: A deep copy.
        """
        if scale is None:
...
class Cylinder(URDFType):
    ...
    def __init__(self, radius, length):
        ...
        :param radius: float, The radius of the cylinder in meters.
        :param length: float, The length of the cylinder in meters.
        """
        self.radius = radius
        self.length = length
        self._meshes = []
    ...
    def meshes(self):
        ...
        """
        if len(self._meshes) == 0:
            self._meshes = [wrs.basis.trimesh.creation.cylinder(
...
class Sphere(URDFType):
    ...
    def __init__(self, radius):
        ...
        :param radius: float, The radius of the sphere in meters.
        """
        self.radius = radius
        self._meshes = []
    ...
    def meshes(self):
        ...
        """
        if len(self._meshes) == 0:
            self._meshes = [wrs.basis.trimesh.creation.icosphere(radius=self.radius)]
        return self._meshes
...
class Mesh(URDFType):
    ...
    def __init__(self, filename, scale=None, meshes=None):
        ...
        :param filename: str, The path to the mesh that contains this object. This can be relative to the top-level URDF
                        or an absolute path.
        :param scale: (3,) float, optional, The scaling value for the mesh along the XYZ axes. If ``None``, assumes no
                        scale is applied.
        :param meshes: list of :class:`~trimesh.base.Trimesh`, A list of meshes that compose this mesh. The list of
                        meshes is useful for visual geometries that might be composed of separate trimesh objects. If not
                        specified, the mesh is loaded from the file using trimesh.
        """
        if meshes is None:
    ...
    def meshes(self, value):
        if isinstance(value, str):
            ...
            for m in value:
                if not isinstance(m, trm.Trimesh):
                    raise TypeError('Mesh requires a trimesh.Trimesh or a '
                                    'list of them')
        elif isinstance(value, trm.Trimesh):
            value = [value]
        else:
    ...
    def _to_xml(self, parent, path):
        ...
        if len(meshes) == 1:
            ...
            meshes = trm.scene.Scene(geometry=meshes)
        trm.exchange.export.export_mesh(meshes, fn)
...
class Geometry(URDFType):
    ...
    def __init__(self, box=None, cylinder=None, sphere=None, mesh=None):
        """
        A wrapper for all geometry types. Only one of the following values can be set, all others should be set to None
        :param box:
        :param cylinder:
        :param sphere:
        :param mesh:
...
class Texture(URDFType):
    ...
    def __init__(self, filename, image=None):
        ...
        :param filename: The path to the image that contains this texture. This can be relative to the top-level URDF or
                        an absolute path.
        :param image: PIL.Image.Image, optional, The image for the texture. If not specified, it is loaded
                        automatically from filename
        """
        if image is None:
...
class Material(URDFType):
    ...
    def __init__(self, name, color=None, texture=None):
        ...
        :param color: the rgba color of the material in the range [0,1].
        :param texture: a texture for the material.
        """
        self.name = name
        self.color = color
...
class Collision(URDFType):
    ...
    def __init__(self, name, origin, geometry):
        ...
        :param name: the name of the collision geometry.
        :param origin: (4,4) the pose of the collision element relative to the link frame. defaults to identity.
...
class Visual(URDFType):
    ...
    def __init__(self, geometry, name=None, origin=None, material=None):
        ...
        :param geometry: the geometry of the element.
        :param name: the name of the visual geometry.
        :param origin: (4,4) the pose of the visual element relative to the link frame. defaults to identity.
...
class Inertial(URDFType):
    ...
    def __init__(self, mass, inertia, origin=None):
        ...
        :param mass: float, the mass of the link in kilograms.
        :param inertia: (3,3) float, the 3x3 symmetric rotational inertia matrix.
        :param origin: (4,4) float, optional, the pose of the inertials relative to the link frame. defaults to identity
        """
        self.mass = mass
...
class JointCalibration(URDFType):
    ...
    def __init__(self, rising=None, falling=None):
        ...
        :param rising: float, when the joint moves in a positive direction, this position will trigger a rising edge
        :param falling: when the joint moves in a positive direction, this position will trigger a falling edge
        """
        self.rising = rising
...
class JointDynamics(URDFType):
    ...
    def __init__(self, damping, friction):
        ...
        :param damping: the values of the joint (Ns/m for prismatic joints, Nms/rad for revolute).
        :param friction: static friction value of the joint (N for prismatic joints, Nm for revolute).
        """
...
class JointLimit(URDFType):
    ...
    def __init__(self, effort, velocity, lower=None, upper=None):
        ...
        :param effort: The maximum joint effort (N for prismatic joints, Nm for revolute).
        :param velocity: max joint velocity (m/s for prismatic joints, rad/s for revolute).
        :param lower: lower joint limit (m for prismatic joints, rad for revolute).
        :param upper: upper joint limit (m for prismatic joints, rad for revolute).
        """
        self.effort = effort
...
class JointMimic(URDFType):
    ...
    def __init__(self, joint, multiplier=None, offset=None):
        ...
        The joint's configuration value is set equal to ``multiplier * other_joint_cfg + offset``.
        :param joint: str, the name of the joint to mimic.
        :param multiplier: float, the joint configuration multiplier. Defaults to 1.0.
        :param offset: float, optional, the joint configuration offset. Defaults to 0.0.
        """
        self.joint = joint
    ...
    def copy(self, prefix='', scale=None):
        """
        create a deep copy with the prefix applied to all names.
        :param prefix: str, a prefix to apply to all joint and link names.
        :param scale:
        :return: a deep copy of the joint mimic.
        """
        return JointMimic(
...
class SafetyController(URDFType):
    ...
    def __init__(self, k_velocity, k_position=None, soft_lower_limit=None, soft_upper_limit=None):
        ...
        :param k_velocity: float, a relation between the effort and velocity limits.
        :param k_position: float, a relation between the position and velocity limits. Defaults to 0.0.
        :param soft_lower_limit: float, the soft lower limit where the safety controller kicks in. Defaults to 0.0.
        :param soft_upper_limit: float, the soft upper limit where the safety controller kicks in. Defaults to 0.0.
        """
...
class Actuator(URDFType):
    ...
    def __init__(self, name, mechanicalReduction=None, hardwareInterfaces=None):
        ...
        An actuator.
        :param name:
        :param mechanicalReduction: str, a specifier for the mechanical reduction at the joint/actuator transmission.
        :param hardwareInterfaces: list of str, the supported hardware interfaces to the actuator.
        """
        self.name = name
...
class TransmissionJoint(URDFType):
    ...
    def copy(self, prefix='', scale=None):
        """
        create a deep copy with the prefix applied to all names.
        :param prefix: str, a prefix to apply to all names.
        :param scale:
        :return: A deep copy.
        """
...
class Transmission(URDFType):
    ...
    def __init__(self, name, trans_type, joints=None, actuators=None):
        ...
        :param name: str
        :param trans_type: str, type of the transmission.
        :param joints: list of TransmissionJoint, the joints connected to this transmission.
        :param actuators: list of Actuator, the actuators connected to this transmission.
        """
        self.name = name
...
class Joint(URDFType):
    ...
    def __init__(self, name, joint_type, parent, child, axis=None, origin=None, limit=None, dynamics=None,
    ...
    def is_valid(self, cfg):
        ...
        :param cfg: float, (2,) float, (6,) float, or (4,4) float, the configuration of the joint.
    ...
    def get_child_pose(self, cfg=None):
        ...
        :param cfg: float, (2,) float, (6,) float, or (4,4) float, the configuration values for this joint.
                    They are interpreted based on the joint type as follows:
                    - fixed - not used.
        ...
        :return: (4,4) float, the pose of the child relative to the parent.
        """
        if cfg is None:
            return self.origin
            ...
            if cfg is None:
                ...
                cfg = float(cfg)
            R = wrs.basis.trimesh.transformations.rotation_matrix(cfg, self.axis)
    ...
    def get_child_poses(self, cfg, n_cfgs):
        ...
        :param cfg: (n,) float or None, the configuration values for this joint.
                    they are interpreted based on the joint type as follows:
            - ``fixed`` - not used.
            - ``prismatic`` - a translation along the axis in meters.
            - ``revolute`` - a rotation about the axis in radians.
    ...
    def _rotation_matrices(self, angles, axis):
        ...
        :param angles: (n, ) float, the angles.
        :param axis: (3, ) float, the axis.
        :return: (n, 4, 4) float, the rotation matrices.
        """
...
class Link(URDFType):
    ...
    def __init__(self, name, inertial, visuals, collisions):
        ...
        :param inertial: the intertial properties of the link.
        :param visuals: the visual properties of the link.
        :param collisions: the collision properties of the link.
        """
        self.name = name
...
class URDF(URDFType):
    ...
    The URDF encapsulates an articulated object, such as a robot or a grippers. It is made of links and joints that tie
    them together and define their relative motions.
    """
    _ATTRIBS = {
        'name': (str, True),
    }
    ...
    def __init__(self, name, links, joints=None, transmissions=None, materials=None, other_xml=None):
        """
        The top-level URDF specification.
        The URDF encapsulates an articulated object, such as a robot or a grippers. It is made of links and joints that
        tie them together and define their relative motions.
        :param name:
        :param links:
        :param joints:
        :param transmissions:
    ...
    def cfg_to_vector(self, cfg):
        ...
        :return: (n,) float, the configuration vector, or None if no actuated joints present.
        """
        if cfg is None:
    ...
    def joint_limit_cfgs(self):
        ...
        The first map is the lower-bound map, which maps limited joints to their lower joint limits.
        The second map is the upper-bound map, which maps limited joints to their upper joint limits.
        """
        lb = {}
        ub = {}
    ...
    def link_fk(self, cfg=None, link=None, links=None, use_names=False):
        ...
        :param cfg: dict or (n), float, a map from joints or joint names to configuration values for each joint,
                    or a list containing a value for each actuated joint in sorted order from the base link.
        :param link: str or Link, a single link or link name to return a pose for.
        :param links: list of str or list of Link, the links or names of links to perform forward kinematics on.
                        only these links will be in the returned map.
                         If neither link nor links are specified all links are returned.
        :param use_names: bool, if True, the returned dictionary will have keys that are string link names
        :return: fk: dict or (4,4) float, a map from links to 4x4 homogenous transform matrices that position them
                    relative to the base link's frame, or a single 4x4 matrix if link is specified.
        """
    ...
    def link_fk_batch(self, cfgs=None, link=None, links=None, use_names=False):
        ...
        :param cfgs:
        :param link: A single link or link name to return a pose for.
        :param links: The links or names of links to perform forward kinematics on.
                        Only these links will be in the returned map. If neither
                        link nor links are specified all links are returned.
        :param use_names: If True, the returned dictionary will have keys that are string
    ...
    def visual_geometry_fk(self, cfg=None, links=None):
        ...
        :param cfg: dict or (n), float, a map from joints or joint names to configuration values for each joint, or
                    a list containing a value for each actuated joint in sorted order from the base link.
                    if not specified, all joints are assumed to be in their default configurations.
        :param links: list of str or list of :class:`.Link`, the links or names of links to perform forward kinematics
                        on. only geometries from these links will be in the returned map. If not specified, all links
                        are returned.
        :return: A map from :class:`Geometry` objects that are part of the visual
    ...
    def copy(self, name=None, prefix='', scale=None, collision_only=False):
        ...
        :param prefix:
        :param scale:
        :param collision_only:  If True, all visual geometry is redirected to the collision geometry.
        :return:
        """
        return URDF(
    ...
    def join(self, other, link, origin=None, name=None, prefix=''):
        ...
        :param other: Another URDF to fuze to this one.
        :param link: The link of this URDF to attach the other URDF to.
        :param origin: The location in this URDF's link frame to attach the base link of the other URDF at.
        :param name: A name for the new URDF.
        :param prefix: If specified, all joints and links from the (other) mesh will be pre-fixed with this value to
                        avoid name clashes.
        :return: The new URDF.
        """
        myself = self.copy()
    ...
    def _validate_joints(self):
        ...
        :return: actuated_joints : list of :class:`.Joint`, the joints in the model that are independently controllable.
        """
        actuated_joints = []
        for joint in self.joints:
    ...
    def _sort_joints(self, joints):
        ...
        :param joints: The joints to sort.
        :return: The sorted joints.
        """
        lens = []
        for joint in joints:
    ...
    def _validate_graph(self):
        ...
        :return: base_link : :class:`.Link`, The base link of the URDF.
                                end_links : list of :class:`.Link`, The end links of the URDF.
        """
    ...
    def _process_cfgs(self, cfgs):
        """Process a list of joint configurations into a dictionary mapping joints to configuration values.
        This should result in a dict mapping each joint to a list of cfg values, one per joint.
        """
