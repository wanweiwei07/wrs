...
# def rayhitmesh_closest(obj, spos, epos, toggledebug=False):
#     mcm = bcdhelper.MCMchecker()
#     pos, nrml = mcm.getRayHitMeshClosest(spos=spos, epos=epos, objcm=obj)
#     if toggledebug:
#         print('------------------')
#         print('spos, epos:', spos, epos)
#         print('pos:', pos)
#         print('normal:', -nrml)
...
def rayhitmesh_p(obj, center, p, direction=np.asarray((0, 0, 1))):
    if abs(direction[0]) == 1:
        pfrom = np.asarray((center[0], p[0] + center[1], p[1] + center[2])) + .05 * direction
        pto = np.asarray((center[0], p[0] + center[1], p[1] + center[2])) - .05 * direction
    elif abs(direction[1]) == 1:
        pfrom = np.asarray((p[0] + center[0], center[1], p[1] + center[2])) + .05 * direction
        pto = np.asarray((p[0] + center[0], center[1], p[1] + center[2])) - .05 * direction
...
def __prj_stroke(stroke, drawcenter, pcd, pcd_nrmls, kdt_d3, mode='DI', objcm=None, pcd_start_p=None, error_method='ED',
    ...
    for i in range(len(stroke) - 1):
        ...
        if mode == 'EI':
            ...
            print("mode name must in ['DI', 'EI', 'QI', 'rbf', 'rbf-g', 'gaussian', 'quad', 'bp']")
...
def get_prj_error(drawpath, pos_nrml_list, method='ED', kdt_d3=None, pcd=None, objcm=None, error_type='mean',
    ...
    if method == 'ED':
        ...
        for i in range(1, len(drawpath)):
            try:
                ...
                iter_times = 1 / step
                prj_len = 0
                gm.gen_sphere(pos=p1, rgba=(1, 0, 0, 1)).attach_to(base)
                while iter_times > 0:
                    p_uv = (pm + np.dot(np.linalg.inv(transmat), v_draw) * step)[:2]
                    z = surface.get_zdata([p_uv])[0]
                    pt = np.asarray([p_uv[0], p_uv[1], z])
                    ...
                    pm = pt
                    iter_times -= 1
                    gm.gen_sphere(pos=pt, rgba=(1, 0, 1, 1)).attach_to(base)
...
def prj_drawpath_ss_on_pcd(obj_item, drawpath, mode='DI', direction=np.asarray((0, 0, 1)), error_method='ED',
    ...
    pos_nrml_list, error, error_list, time_cost = \
        __prj_stroke(drawpath, obj_item.drawcenter, obj_item.mph, obj_item.nrmls, kdt_d3, objcm=obj_item.objcm,
    ...
    for p in drawpath_stroke_scaled:
        ...
        for face_id, face in enumerate(faces):
            ...
            if cu.is_in_polygon(p_uv, polygon):
                ...
                v_draw = (v_draw[0], v_draw[1], 0)
                rotmat = rm.rotmat_between_vectors(np.array([0, 0, 1]), nrmls[face_id])
...
def show_drawpath(pos_nrml_list, color=(1, 0, 0), show_nrmls=False, transparency=1.0):
    for i in range(len(pos_nrml_list)):
        ...
        if p is not None:
            ...
            gm.gen_sphere(pos=np.array(p), radius=.001, rgba=(color[0], color[1], color[2], 1))
            if show_nrmls:
                gm.gen_arrow(spos=p, epos=p+.01*n, rgba=(color[0], color[1], color[2], transparency))
...
def get_penmat4(pos_nrml_list, pen_orgrot=(1, 0, 0)):
    ...
    for p, n in pos_nrml_list:
        ...
        #     rotmat = rm.rotmat_between_vectors(pen_orgrot, -n)
        # else:
        #     rotmat = rm.rotmat_between_vectors(pen_orgrot, n)
...
if __name__ == '__main__':
    ...
    # ray hit mesh
    # time_1 = time.time()
    # pos_nrml_list, error_list = rayhitmesh_drawpath_ss(tgt_item, drawpath, toggledebug=True)
    # show_drawpath(pos_nrml_list, color=(0, 1, 0))
    # print('rayhit time cost', time.time() - time_1)
    #
    # # conformal mapping
    # pos_nrml_list, error_list_cm, time_cost = prj_drawpath_ss_SI(tgt_item, drawpath)
    # show_drawpath(pos_nrml_list, color=(0, 1, 0))
    ...
    # # drawpath = du.gen_square(side_len=40, step=1)
    # drawpath = resize_drawpath(drawpath, 30, 30, space=0)
    #
    # # loop_pos_nrml_list, loop_error_list, loop_time_cost = \
    # #     prj_drawpath_ss_SI_loop(tgt_item, drawpath, toggledebug=False, step=1)
    # # dump_mapping_res(f'{dump_f_name}_circle_SI.pkl', tgt_item, drawpath, loop_pos_nrml_list, loop_time_cost)
    #
    # loop_pos_nrml_list, loop_error_list, loop_time_cost = \
    #     prj_drawpath_ss_loop(tgt_item, drawpath, mode='EI', toggledebug=False, step=1)
    # dump_mapping_res(f'{dump_f_name}_circle_SI.pkl', tgt_item, drawpath, loop_pos_nrml_list, loop_time_cost)
    # show_drawpath(loop_pos_nrml_list[0], color=(1, 0, 1))
    # show_drawpath(loop_pos_nrml_list[1], color=(0, 1, 1))
    # show_drawpath(loop_pos_nrml_list[2], color=(0, 0, 1))
    # show_drawpath(loop_pos_nrml_list[3], color=(0, 1, 0))
    # base.run()
    ...
    error_method = 'ED'
    # ray hit mesh
    # pos_nrml_list, error_list_rh, time_cost_total = \
    #     rayhitmesh_drawpath_ms(tgt_item, drawpath_ms, error_method=error_method)
    # print('rayhit time cost', time_cost_total)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 1, 0))
    # dump_mapping_res(f'{dump_f_name}_baseline.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost_total)
    # base.run()

    # conformal mapping
    # pos_nrml_list, error_list_cm, time_cost = \
    #    prj_drawpath_ms_cmap_mesh(tgt_item, drawpath_ms, toggledebug=True)
    # dump_mapping_res(f'{dump_f_name}_SI.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(1, 0, 0), show_nrmls=False)

    # pos_nrml_list, error_list_cm, time_cost = \
    #     prj_drawpath_ms_cmap_temp(tgt_item, drawpath_ms, sample_num=SAMPLE_NUM, toggledebug=False)
    # # pos_nrml_list, error_list_cm, time_cost = \
    # #     prj_drawpath_ms_cmap(tgt_item, drawpath_ms, toggledebug=False)
    # dump_mapping_res(f'{dump_f_name}_II.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 0, 1))
    # base.run()

    # metrology based
    # pos_nrml_list, error_list_di, time_cost = \
    #     prj_drawpath_ms_on_pcd(tgt_item, drawpath_ms, mode='DI', step=1, error_method=error_method,
    #                            toggledebug=False)
    ...
    # if SAMPLE_NUM <= 100000 or SAMPLE_NUM is None:
    #     dump_mapping_res(f'{dump_f_name}_DI\'.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # else:
    #     dump_mapping_res(f'{dump_f_name}_DI.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # base.run()

    # pos_nrml_list, error_list_qi, time_cost = \
    #     prj_drawpath_ms_on_pcd(tgt_item, drawpath_ms, mode='QI', step=1, error_method=error_method,
    #                            toggledebug=False)
    # print('metrology method time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 0, 1), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_QI.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    ...
    dump_mapping_res(f'{dump_f_name}_gaussian.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    base.run()
    ...
    # dump_mapping_res(f'{dump_f_name}_s5_EI.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    #
    # pos_nrml_list, _, time_cost = \
    ...
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(1, 0, 0), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_s5_snap_quad.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    #
    # pos_nrml_list, _, time_cost = \
    ...
    # dump_mapping_res(f'{dump_f_name}_s5_snap_RBF.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    ...
    # print('rbf time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 0, 1), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_s20_RBF-G.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # base.run()
    ...
    # print('rbf time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(1, 0, 0), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_RBF-G-linear.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # base.run()
    ...
    # print('rbf time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 0, 1), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_bp.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # base.run()

    # # show DRAW
    # drawpath_ms_D = resize_drawpath_ms(du.load_drawpath('D.pkl'), 20, 20, space=0)
    # drawpath_ms_R = resize_drawpath_ms(du.load_drawpath('R.pkl'), 20, 20, space=0)
    # drawpath_ms_A = resize_drawpath_ms(du.load_drawpath('A.pkl'), 20, 20, space=0)
    # drawpath_ms_W = resize_drawpath_ms(du.load_drawpath('W.pkl'), 20, 20, space=0)
    # # +40, +15, +2, -13
    # tgt_item.set_drawcenter((-10, 40, 0))
    #
    # pos_nrml_list, _ = prj_drawpath_ms_on_pcd(tgt_item, drawpath_ms_D, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # tgt_item.set_drawcenter((0, -20, 0))
    # pos_nrml_list, _ = prj_drawpath_ms_on_pcd(tgt_item, drawpath_ms_R, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # tgt_item.set_drawcenter((0, -13, 0))
    #
    # pos_nrml_list, _ = prj_drawpath_ms_on_pcd(tgt_item, drawpath_ms_A, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # tgt_item.set_drawcenter((5, -15, 0))
    # pos_nrml_list, _ = prj_drawpath_ms_on_pcd(tgt_item, drawpath_ms_W, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # base.run()
