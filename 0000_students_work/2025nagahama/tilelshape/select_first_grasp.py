    ...
    for grasp2 in grasp_groups[i2].grasps:
        ...
        for grasp3 in grasp_groups[i3].grasps:
            ...
            mgm.gen_box(xyz_lengths=rm.vec(.3, 0.001, .3), pos=grasp1.ac_pos, rotmat=grasp1.ac_rotmat,
                        rgb=grasp_groups[i1].color, alpha=.5).attach_to(base)
            # remaining uncertainty
            for i in range(100):
                obj_cmodel_copy = obj_cmodel.copy()
                rot_randomness = rm.np.random.randn(1) * rm.pi / 18
                rotmat = rm.rotmat_from_axangle(grasp1.ac_rotmat[:, 1], rot_randomness)
                obj_cmodel_copy.rotmat = rotmat
                obj_cmodel_copy.pos = rotmat @ (obj_cmodel_copy.pos +.007 * rm.np.random.randn(3)- grasp1.ac_pos) + grasp1.ac_pos
                obj_cmodel_copy.alpha = .5
                if gripper.is_mesh_collided(obj_cmodel_copy):
                    continue
                obj_cmodel_copy.attach_to(base)
            # # show grasp2
            # gripper.grip_at_by_pose(jaw_center_pos=grasp2.ac_pos, jaw_center_rotmat=grasp2.ac_rotmat,jaw_width=grasp2.ee_values)
