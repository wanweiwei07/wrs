...
sim_mat = rm.np.clip(sim_mat, 0.0, 1.0)
dist_mat = 1 - sim_mat
...
for grasp1 in grasp_groups[i1].grasps:
    ...
    for grasp2 in grasp_groups[i2].grasps:
        ...
        for grasp3 in grasp_groups[i3].grasps:
            ...
            # show grasp1
            gripper.grip_at_by_pose(jaw_center_pos=grasp1.ac_pos, jaw_center_rotmat=grasp1.ac_rotmat,
                                    jaw_width=grasp1.ee_values)
            gripper.gen_meshmodel(rgb=grasp_groups[i1].color, alpha=1).attach_to(base)
            ...
            rotmat = rm.rotmat_from_axangle(grasp1.ac_rotmat[:, 1], rm.pi / 18)
            obj_cmodel_copy.rotmat = rotmat
            obj_cmodel_copy.pos = rotmat @ (
                        obj_cmodel_copy.pos + grasp1.ac_rotmat[:, 0]*0.0005+grasp1.ac_rotmat[:, 1]*0.001 - grasp1.ac_pos) + grasp1.ac_pos
            obj_cmodel_copy.rgb = grasp_groups[i1].color
            obj_cmodel_copy.alpha = 1
            # if gripper.is_mesh_collided(obj_cmodel_copy):
            #     continue
            obj_cmodel_copy.attach_to(base)
            # # show grasp2 assumed
            gripper.grip_at_by_pose(jaw_center_pos=grasp2.ac_pos, jaw_center_rotmat=grasp2.ac_rotmat,
                                    jaw_width=grasp2.ee_values)
            gripper.gen_meshmodel(rgb=grasp_groups[i2].color, alpha=0.5).attach_to(base)
            ...
            # # show grasp2 with uncertainty
            rel_pos, rel_rotmat = rm.rel_pose(obj_cmodel.pos, obj_cmodel.rotmat, obj_cmodel_copy.pos,
                                              obj_cmodel_copy.rotmat)
            new_grasp_ac_pos = rel_rotmat @ grasp2.ac_pos + rel_pos
            new_grasp_ac_rotmat = rel_rotmat @ grasp2.ac_rotmat
            gripper.grip_at_by_pose(jaw_center_pos=new_grasp_ac_pos,
                                    jaw_center_rotmat=new_grasp_ac_rotmat, jaw_width=grasp2.ee_values)
            gripper.gen_meshmodel(rgb=grasp_groups[i2].color, alpha=1).attach_to(base)
            # intersection of two planes
            pnt, dir = rm.intersect_planes(grasp1.ac_pos, grasp1.ac_rotmat[:, 1], new_grasp_ac_pos, new_grasp_ac_rotmat[:, 1])
            mgm.gen_arrow(pnt, pnt+dir*.3, alpha=.3).attach_to(base)
            mgm.gen_arrow(pnt, pnt-dir*.3, alpha=.3).attach_to(base)
            mgm.gen_box(xyz_lengths=rm.vec(.3, 0.001, .3), pos=new_grasp_ac_pos, rotmat=new_grasp_ac_rotmat,
                        rgb=grasp_groups[i2].color, alpha=.3).attach_to(base)
            # # show grasp3
            # gripper.grip_at_by_pose(jaw_center_pos=grasp3.ac_pos, jaw_center_rotmat=grasp3.ac_rotmat,jaw_width=grasp3.ee_values)
            # gripper.gen_meshmodel(rgb=grasp_groups[i3].color, alpha=0.5).attach_to(base)
