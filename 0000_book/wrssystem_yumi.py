...
if __name__ == "__main__":
    ...
    tic = time.time()
    # jnt_values = robot_s.ik(hand_name, tgt_pos, tgt_rotmat, seed_jnt_values=np.array([.0,.0,.0,.0,.0,.0,.0]))
    jnt_values = yumi_s.ik(manipulator_name, tgt_pos, tgt_rotmat)
    # jnt_values1 = robot_s.ik(hand_name, tgt_pos1, tgt_rotmat1)
    # jnt_values2 = robot_s.ik(hand_name, tgt_pos2, tgt_rotmat2)
