...
class Motor(object):
    ...
    def gen_meshmodel(self,
        ...
        if toggle_stator_frame:
            mgm.gen_frame(pos=self._pos + self._rotmat @ in_stator_offset, rotmat=self._rotmat @ in_rotmat,
                          ax_length=.05, ax_radius=.001).attach_to(m_col)
        if toggle_rotor_frame:
            mgm.gen_frame(pos=self._pos + self._rotmat @ in_rotor_offset, rotmat=self._rotmat @ in_rotmat,
                          ax_length=.05, ax_radius=.001).attach_to(m_col)
            mgm.gen_dashed_frame(pos=self._pos + self._rotmat @ in_rotor_offset,
...
if option == 'a':
    # visualize
    base = wd.World(cam_pos=np.array([.3, .3, .3]), lookat_pos=np.array([0, 0.025, 0]))
...
if option == 'c':
    ...
    nxt_motor.gen_meshmodel(toggle_link=True, toggle_rotax=False, toggle_rotor_frame=True,
                            toggle_stator_frame=True, alpha=.3).attach_to(base)
    motor.motion_value = np.pi/6
    nxt_loc_pos = rm.rotmat_from_euler(0, 0, np.pi /2) @ np.array([0, 0.121, 0])
    nxt_pos = motor.pos + motor.rotmat @ rm.rotmat_from_euler(0, 0, motor.motion_value) @ nxt_loc_pos
    nxt_rotmat = motor.rotmat @ rm.rotmat_from_euler(0, 0, motor.motion_value)
