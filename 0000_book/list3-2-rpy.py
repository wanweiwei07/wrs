...
if option == 'x':
    ...
    mgm.gen_circarrow(axis=rm.vector(1, 0, 0),
                      portion=.9,
                      center=rm.vector(.1, 0, 0),
                      major_radius=.03,
                      minor_radius=.0015,
                      rgb=rm.vector(.3, .3, .3),
                      alpha=1).attach_to(base)
    draw_rot_arrows(rotmat=rm.eye(3), axis=rm.vector(1, 0, 0), column_id=1, rgb=rm.const.green, portion=1 / 6)
    draw_rot_arrows(rotmat=rm.eye(3), axis=rm.vector(1, 0, 0), column_id=2, rgb=rm.const.blue, portion=1 / 6)
    base.run()
if option == 'xy':
    mgm.gen_frame(ax_length=.2, alpha=rm.vector(.1, .1, .1)).attach_to(base)
    rotmat_oa = rm.rotmat_from_euler(rm.pi / 3, 0, 0)
    mgm.gen_dashed_frame(ax_length=.2, rotmat=rotmat_oa, len_solid=.06, len_interval=.01).attach_to(base)
    ...
    mgm.gen_circarrow(axis=rm.vector(0, -1, 0),
                      portion=.9,
                      center=rm.vector(0, .1, 0),
                      major_radius=.03,
                      minor_radius=.0015,
                      rgb=rm.vector(.3, .3, .3),
                      alpha=1).attach_to(base)
    draw_rot_arrows(rotmat=rotmat_oa, axis=rm.vector(0, -1, 0), column_id=0, rgb=rm.const.red, portion=1 / 12)
    draw_rot_arrows(rotmat=rotmat_oa, axis=rm.vector(0, -1, 0), column_id=1, rgb=rm.const.green, portion=1 / 12)
    draw_rot_arrows(rotmat=rotmat_oa, axis=rm.vector(0, -1, 0), column_id=2, rgb=rm.const.blue, portion=1 / 12)
    base.run()
if option == 'xyz':
    mgm.gen_frame(ax_length=.2, alpha=rm.vector(.1, .1, .1)).attach_to(base)
    rotmat_ab = rm.rotmat_from_euler(rm.pi / 3, -rm.pi / 6, 0)
    ...
    mgm.gen_circarrow(axis=rm.vector(0, 0, 1),
                      portion=.9,
                      center=rm.vector(0, 0, .1),
                      major_radius=.03,
                      minor_radius=.0015,
                      rgb=rm.vector(.3, .3, .3),
                      alpha=1).attach_to(base)
    draw_rot_arrows(rotmat=rotmat_ab, axis=rm.vector(0, 0, 1), column_id=0, rgb=rm.const.red, portion=1 / 6)
    draw_rot_arrows(rotmat=rotmat_ab, axis=rm.vector(0, 0, 1), column_id=1, rgb=rm.const.green, portion=1 / 6)
    draw_rot_arrows(rotmat=rotmat_ab, axis=rm.vector(0, 0, 1), column_id=2, rgb=rm.const.blue, portion=1 / 6)
    base.run()
if option == 'final':
    frame_o = mgm.gen_frame(ax_length=.2, alpha=rm.vector(1, 1, 1))
