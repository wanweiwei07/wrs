...
if option == 'a':
    ...
    rotmat_start = rm.rotmat_from_axangle(ax, -np.pi / 6)
    cross_vec = rotmat_start.dot(rotmat[:, 0])
    rotmat_goal = rm.rotmat_from_axangle(ax, np.pi / 3)
    mgm.gen_arrow(epos=ax * .3, rgb=rm.bc.gray, stick_radius=.0025, alpha=.9).attach_to(base)
    mgm.gen_frame(ax_length=.2).attach_to(base)
    mgm.gen_arrow(epos=cross_vec * .3, rgb=rm.bc.yellow, alpha=1).attach_to(base)
    nxt_vec_uvw = rotmat_goal.dot(cross_vec)
    mgm.gen_dashed_arrow(epos=nxt_vec_uvw * .3, rgb=rm.bc.yellow, alpha=1).attach_to(base)
    radius, _ = rm.unit_vector(cross_vec * .3 - cross_vec.dot(ax) * ax * .3, toggle_length=True)
    mgm.gen_arrow(spos=cross_vec.dot(ax) * ax * .3, epos=cross_vec * .3, rgb=rm.bc.orange, alpha=1).attach_to(base)
    mgm.gen_dashed_arrow(spos=cross_vec.dot(ax) * ax * .3, epos=nxt_vec_uvw * .3, rgb=rm.bc.orange,
                         alpha=1).attach_to(base)
    mgm.gen_arrow(epos=ax * np.sqrt(.3 ** 2 - radius ** 2), stick_radius=.003, rgb=rm.bc.black, alpha=1).attach_to(base)
    mgm.gen_sphere(radius=.005, pos=ax * np.sqrt(.3 ** 2 - radius ** 2), rgb=rm.bc.black, alpha=1).attach_to(base)
    mgm.gen_sphere(radius=.005, pos=cross_vec * .3, rgb=rm.bc.black, alpha=1).attach_to(base)
    mgm.gen_sphere(radius=.005, pos=nxt_vec_uvw * .3, rgb=rm.bc.black, alpha=1).attach_to(base)
    # rectangle
    ...
    mgm.gen_torus(ax,
                  ...
                  minor_radius=.0015,
                  n_sec_major=64,
                  rgb=rm.bc.white, alpha=1).attach_to(base)
    mgm.gen_circarrow(ax,
                      ...
                      n_sec_major=64,
                      minor_radius=.0015,
                      rgb=rm.bc.orange, alpha=1).attach_to(base)
if option == 'b':
    rotmat = rm.rotmat_from_euler(np.pi / 3, -np.pi / 6, np.pi / 3)
    ax, angle = rm.axangle_between_rotmat(np.eye(3), rotmat)
...
if option == 'c':
    ...
    mgm.gen_dashed_arrow(spos=ax * np.sqrt(.3 ** 2 - radius ** 2),
                         epos=ax * np.sqrt(.3 ** 2 - radius ** 2) + np.cross(ax, cross_vec * .3) * np.sin(np.pi / 3),
                         rgb=np.array([1, 0, 1]), alpha=.5).attach_to(base)
    mgm.gen_dashed_arrow(spos=ax * np.sqrt(.3 ** 2 - radius ** 2),
                         epos=ax * np.sqrt(.3 ** 2 - radius ** 2) + (
                                 cross_vec * .3 - cross_vec.dot(ax) * ax * .3) * np.cos(np.pi / 3),
                         rgb=np.array([0, 1, 1]), alpha=.5).attach_to(base)
    # lines
    mgm.gen_dashed_stick(spos=nxt_vec_uvw * .3,
                         epos=ax * np.sqrt(.3 ** 2 - radius ** 2) + np.cross(ax, cross_vec * .3) * np.sin(np.pi / 3),
                         radius=.001, rgb=rm.bc.black, alpha=1, len_solid=.01, len_interval=.007).attach_to(base)
    mgm.gen_dashed_stick(spos=nxt_vec_uvw * .3,
                         epos=ax * np.sqrt(.3 ** 2 - radius ** 2) + (
                                 cross_vec * .3 - cross_vec.dot(ax) * ax * .3) * np.cos(np.pi / 3),
                         radius=.001, rgb=rm.bc.black, alpha=1, len_solid=.01, len_interval=.007).attach_to(base)
