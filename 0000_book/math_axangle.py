...
# gm.gen_sphere(major_radius=.2, rgba=[.67,.67,.67,.9], sphere_ico_level=5).attach_to(base)
#
# cross_vec = rm.unit_vector(np.cross(np.array([0,0,1]), rotmat[:3,2]))
# # gm.gen_dasharrow(epos=cross_vec*.2, rgba=[1,1,0,1], len_solid=.01, len_interval=.0077).attach_to(base)
# gm.gen_arrow(epos=cross_vec*.2, rgba=[1,1,0,1]).attach_to(base)
# gm.gen_sphere(major_radius=.005, pos=cross_vec*.2, rgba=[0,0,0,1]).attach_to(base)
# #
# nxt_vec_uvw = rotmat.dot(cross_vec)
# # gm.gen_dasharrow(epos=nxt_vec_uvw*.2, rgba=[1,1,0,1], len_solid=.015, len_interval=.007).attach_to(base)
# gm.gen_arrow(epos=nxt_vec_uvw*.2, rgba=[1,1,0,1]).attach_to(base)
# gm.gen_sphere(major_radius=.005, pos=nxt_vec_uvw*.2, rgba=[0,0,0,1]).attach_to(base)
# #
# pre_vec_xyz = rotmat.T.dot(cross_vec)
# gm.gen_arrow(epos=pre_vec_xyz*.2, rgba=[1,1,0,1]).attach_to(base)
# gm.gen_sphere(major_radius=.005, pos=pre_vec_xyz*.2, rgba=[0,0,0,1]).attach_to(base)
#
...
# print(major_radius)
# gm.gen_circarrow(ax, portion=1, center=cross_vec.dot(ax)*ax*.2, major_radius=major_radius, n_sec_major=64, n_sec_minor=16, major_radius=.003, rgba=[0,0,0,1]).attach_to(base)

# print(ax)
# vec=np.array([1,0,0])
# major_radius, _ = rm.unit_vector(vec*.2-vec.dot(ax)*ax*.2, toggle_length=True)
# gm.gen_circarrow(ax, starting_vector=vec-vec.dot(ax)*ax, portion=.57, center=vec.dot(ax)*ax*.2, major_radius=major_radius, n_sec_major=64, n_sec_minor=16, major_radius=.003, rgba=[1,0,0,1]).attach_to(base)
# vec=np.array([0,1,0])
# major_radius, _ = rm.unit_vector(vec*.2-vec.dot(ax)*ax*.2, toggle_length=True)
# gm.gen_circarrow(ax, portion=1, center=vec.dot(ax)*ax*.2, major_radius=major_radius, n_sec_major=64, n_sec_minor=16, major_radius=.003, rgba=[0,1,0,1]).attach_to(base)
# vec=np.array([0,0,1])
# major_radius, _ = rm.unit_vector(vec*.2-vec.dot(ax)*ax*.2, toggle_length=True)
# gm.gen_circarrow(ax, starting_vector=vec-vec.dot(ax)*ax, portion=.57, center=vec.dot(ax)*ax*.2, major_radius=major_radius, n_sec_major=64, n_sec_minor=16, major_radius=.003, rgba=[0,0,1,1]).attach_to(base)
