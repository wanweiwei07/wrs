...
def gen_stand():
    ...
    hinge = (mgm.gen_stick(spos=rm.vec_from_args(.015, 0, 0),
                           epos=rm.vec_from_args(-.015, 0, 0),
                           radius=.015,
                           rgb=rm.const.steel_gray))
    hinge.alpha = .5
    hinge.attach_to(m_col)
    holder = mgm.gen_box(xyz_lengths=rm.vec_from_args(.03, .03, .02),
                         pos=rm.vec_from_args(0, 0, -.01),
                         rgb=rm.const.steel_gray)
    holder.alpha = .5
    holder.attach_to(m_col)
    stand = mgm.gen_stick(spos=rm.vec_from_args(0, 0, -0.02),
                          epos=rm.vec_from_args(0, 0, -0.0225),
                          radius=0.03,
                          rgb=rm.const.steel_gray)
    stand.alpha = .5
...
if __name__ == '__main__':
    ...
    base = wd.World(cam_pos=rm.vec_from_args(1, 0, .1), lookat_pos=rm.vec_from_args(0, 0, 0.1))
    gen_stand().attach_to(base)
    mgm.gen_frame().attach_to(base)

    jlc = rkjlc.JLChain(n_dof=2)
    jlc.jnts[0].loc_pos = rm.vec_from_args(0, 0, 0)
    jlc.jnts[0].loc_motion_ax = rm.vec_from_args(1, 0, 0)
    jlc.jnts[1].loc_pos = rm.vec_from_args(0, 0, .1)
    jlc.jnts[1].loc_motion_ax = rm.vec_from_args(1, 0, 0)
    jlc._loc_flange_pos = rm.vec_from_args(0, 0, .1)
    jlc.finalize()
