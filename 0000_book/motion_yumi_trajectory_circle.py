...
if __name__ == "__main__":
    ...
    # robot_s.gen_meshmodel(toggle_tcp_frame=False, rgba=[.7,.3,.3,.57]).attach_to(base)
    # robot_s.fk(hnd_name, jnt_values_list[2])
    # robot_s.gen_meshmodel(toggle_tcp_frame=False, rgba=[.3,.7,.3,.57]).attach_to(base)
    # robot_s.fk(hnd_name, jnt_values_list[3])
    # robot_s.gen_meshmodel(toggle_tcp_frame=False, rgba=[.3,.3,.7,.57]).attach_to(base)
