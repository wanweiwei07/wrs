import robot_sim.kinematics.jlchain as lib_jlc
import robot_sim.kinematics.jl as lib_jl
import robot_sim.kinematics.model_generator as lib_jlm
...
if __name__ == '__main__':
    ...
    jlc.joints[3].change_type(type=lib_jl.JointType.PRISMATIC, motion_range=np.array([-1, 1]))
    jlc.joints[3].loc_pos = np.array([0, 0, .1])
    jlc.joints[3].loc_motion_axis = np.array([0, 0, 1])
    jlc.joints[4].loc_pos = np.array([0, 0, .1])
    jlc.joints[4].loc_motion_axis = np.array([0, 0, 1])
