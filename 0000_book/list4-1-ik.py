...
from wrs import basis as rm, robot_sim as rkmg, robot_sim as rkjlc, robot_sim as rkc, modeling as mgm
...
if option == 'a':
    ...
    jlc.goto_given_conf(
        jnt_values=np.array([np.pi / 12 - np.pi / 18, -np.pi / 3, np.pi * 2 / 3, np.pi / 12, np.pi / 3, 0]))
...
if option == 'b':
    jlc.goto_given_conf(
        jnt_values=np.array([np.pi / 12, -np.pi / 3, np.pi * 2 / 3, np.pi / 12, np.pi / 3, 0]))
    jlc.gen_stickmodel(toggle_jnt_frames=False, toggle_actuation=False, lnk_alpha=.2, jnt_alpha=.2).attach_to(base)
    prev_pos = jlc.gl_flange_pos
