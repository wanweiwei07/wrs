...
def draw_coord(pnt, rotmat=np.eye(3), toggle_pnt=False, toggle_coord=False):
    ...
    if toggle_pnt:
        mgm.gen_sphere(pos=global_pnt, radius=.005, rgba=rgba).attach_to(base)
    if toggle_coord:
    ...
    mgm.gen_stick(spos=global_pnt, epos=rotmat @ np.array([px, py, 0]), radius=0.001, rgba=rgba).attach_to(base)
    mgm.gen_dashed_stick(spos=rotmat @ np.array([px, py, 0]), epos=rotmat @ np.array([px, 0, 0]), radius=0.001,
                         rgba=rgba, len_solid=.007,
    ...
    mgm.gen_dashed_stick(spos=rotmat @ np.array([px, py, 0]), epos=rotmat @ np.array([0, py, 0]), radius=0.001,
                         rgba=rgba, len_solid=.007,
                         len_interval=.005).attach_to(base)
    mgm.gen_stick(spos=global_pnt, epos=rotmat @ np.array([px, 0, pz]), radius=0.001, rgba=rgba).attach_to(base)
    ...
    mgm.gen_dashed_stick(spos=rotmat @ np.array([px, 0, pz]), epos=rotmat @ np.array([0, 0, pz]), radius=0.001,
                         rgba=rgba, len_solid=.007,
                         len_interval=.005).attach_to(base)
    mgm.gen_stick(spos=global_pnt, epos=rotmat @ np.array([0, py, pz]), radius=0.001, rgba=rgba).attach_to(base)
...
if __name__ == '__main__':
    ...
    print(np.degrees(rm.rotmat_to_euler(o_r_a)))
