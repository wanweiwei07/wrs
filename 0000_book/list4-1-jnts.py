...
from wrs import basis as rm, robot_sim as rkmg, robot_sim as rkc, modeling as mgm
...
if option == 'c':
    ...
    jnt_1.loc_pos = np.array([-.121, 0, 0])
    ...
    mgm.gen_stick(spos=jnt_0.gl_pos_0, epos=jnt_1.gl_pos_0, rgb=rm.bc.lnk_stick_rgba,
                         radius=rkc.LNK_STICK_RADIUS,alpha=.7).attach_to(base)
    ...
    mgm.gen_stick(spos=jnt_0.gl_pos_0, epos=jnt_1.gl_pos_0, rgb=rm.bc.lnk_stick_rgba,
                  radius=rkc.LNK_STICK_RADIUS, alpha=.2).attach_to(base)
    mgm.gen_stick(spos=jnt_0.gl_pos_0+jnt_0.gl_rotmat_0[:,0], epos=jnt_0.gl_pos_0, rgb=rm.bc.lnk_stick_rgba,
                         radius=rkc.LNK_STICK_RADIUS,alpha=.7).attach_to(base)
