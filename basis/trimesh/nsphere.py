...
def minimum_nsphere(obj):
    ...
    Uses the fact that the minimum n- sphere will be centered at one of the vertices of the furthest site voronoi
    diagram, which is n*log(n) but should be pretty fast due to using the scipy/qhull implementations  of convex hulls
    and voronoi diagrams.
    :param obj: (n, d) float or trimesh.Trimesh, Points or mesh to find minimum bounding nsphere
    :return: center : (d,) float
              Center of fitted n- sphere
             radius : float
              Radius of fitted n-sphere
    """
    # reduce the input points or mesh to the vertices of the convex hull
    # since we are computing the furthest site voronoi diagram this reduces the input complexity substantially and
    # returns the same value
    points = convex.hull_points(obj)
    # we are scaling the mesh to a unit cube
    # this used to pass qhull_options 'QbB' to Voronoi
    # however this had a bug somewhere to avoid this we scale to a unit cube ourselves inside this function
    points_origin = points.min(axis=0)
    points_scale = np.ptp(points, axis=0).min()
    points = (points - points_origin) / points_scale
    # if all of the points are on an n-sphere already the voronoi method will fail so we check a least squares fit
    # before bothering to compute the voronoi diagram
    ...
    # this will fail if the points are ALL on the surface of the n-sphere but hopefully the least squares check caught
    # those cases, qhull_options='QbB Pp')
    voronoi = spatial.Voronoi(points, furthest_site=True)
    # find the maximum radius^2 point for each of the voronoi vertices
    # this is worst case quite expensive but we have taken convex hull to reduce n for this operation we are doing
    # comparisons on the radius squared then rooting once
    try:
        # cdist is massivly faster than looping or tiling methods although it does create a very large intermediate
        # array first, get an order of magnitude memory size estimate a float64 would be 8 bytes per entry plus overhead
...
def fit_nsphere(points, prior=None):
    ...
    :param points: (n, d) float, points in space
    :param prior: (d,) float, best guess for center of nsphere
    :return: center: (d,) float, location of center
    ...
    def residuals(center):
        # do the axis sum with a dot, this gets called a LOT so worth optimizing
        radii_sq = np.dot((points - center) ** 2, ones)
        # residuals are difference between mean, use our sum mean vs .mean() as it is slightly faster
        return radii_sq - (radii_sq.sum() / len(radii_sq))

    guess = points.mean(axis=0) if prior is None else np.asanyarray(prior)
...
def is_nsphere(points):
    ...
    :param points: (n, dimension) float, points in space
    :return: bool, true if input points are on an nsphere
    """
