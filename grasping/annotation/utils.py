...
def define_gripper_grasps_with_rotation(gripper, cmodel, gl_jaw_center_pos, gl_approaching_vec, gl_fgr0_opening_vec,
    ...
    for rotate_angle in np.arange(rotation_range[0], rotation_range[1], rotation_interval):
        tmp_rotmat = rm.rotmat_from_axangle(gl_fgr0_opening_vec, rotate_angle)
        tmp_approaching_vec = np.dot(tmp_rotmat, gl_approaching_vec)
    ...
    if toggle_debug:
        for grasp_info in collided_grasp_info_list:
            jaw_width, gl_jaw_center_pos, gl_jaw_center_rotmat, gripper_root_pos, gripper_root_rotmat = grasp_info
            gripper.fix_to(gripper_root_pos, gripper_root_rotmat)
...
if __name__ == '__main__':
    ...
    grasp_info_list = define_gripper_grasps_with_rotation(gripper_s, objcm, gl_jaw_center_pos=np.array([0, 0, 0]),
                                                          gl_approaching_vec=np.array([1, 0, 0]),
                                                          gl_fgr0_opening_vec=np.array([0, 1, 0]), jaw_width=.04)
    for grasp_info in grasp_info_list:
