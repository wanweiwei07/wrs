...
class SingleContactDeviceInterface(object):
    ...
    def __init__(
        ...
        # toggle on the following part
        # to assign an explicit mesh model to a coupling
        # self.coupling.lnks[0]['meshfile'] = \
        #    os.path.join(this_dir, "meshes", "xxx.stl")
        # self.coupling.lnks[0]['rgba'] = [.2, .2, .2, 1]
        self.coupling.reinitialize()
        # single contact device center
        self.single_contact_device_center_pos = np.zeros(3)
        self.single_contact_device_center_rotmat = np.eye(3)
        # collision detection
        self.cc = None
    ...
    def is_collided(self, obstacle_list=[], otherrobot_list=[]):
        """
        Interface for "is cdprimit collided"
        :param obstacle_list:
        ...
        date: 20201223
        """
        return_val = self.cc.is_collided(
    ...
    def is_mesh_collided(self, objcm_list=[], toggle_debug=False):
        for i, cdelement in enumerate(self.all_cdelements):
            ...
            iscollided, collided_points = \
                self.cdmesh_collection.cm_list[i].is_mcdwith(objcm_list, True)
            if iscollided:
                if toggle_debug:
    ...
    def single_contact_device_to_with_dcpose(
            self,
            gl_single_contact_device_center_pos,
            gl_single_contact_device_center_rotmat):
        """
        ...
        :return:
        """
        eef_root_rotmat = gl_single_contact_device_center_rotmat.dot(
            self.single_contact_device_center_rotmat.T)
        eef_root_pos = \
            gl_single_contact_device_center_pos - eef_root_rotmat.dot(self.single_contact_device_center_pos)
        self.fix_to(eef_root_pos, eef_root_rotmat)
        return [gl_single_contact_device_center_pos,
                gl_single_contact_device_center_rotmat,
                eef_root_pos,
                eef_root_rotmat]

    def single_contact_device_to_with_dczy(
            self,
            gl_single_contact_device_center_pos,
            gl_single_contact_device_center_z,
            gl_single_contact_device_center_y):
        """
        ...
        gl_jaw_center_rotmat[:, 2] = \
            rm.unit_vector(gl_single_contact_device_center_z)
        gl_jaw_center_rotmat[:, 1] = \
            rm.unit_vector(gl_single_contact_device_center_y)
        gl_jaw_center_rotmat[:, 0] = \
            np.cross(gl_jaw_center_rotmat[:3, 1], gl_jaw_center_rotmat[:3, 2])
        return self.single_contact_device_to_with_dcpose(
            gl_single_contact_device_center_pos, gl_single_contact_device_center_z)
    ...
    def gen_stickmodel(self,
                       ...
                       name='single_contact_device_stickmodel'):
        raise NotImplementedError
    ...
    def gen_meshmodel(self,
                      ...
                      toggle_jntscs=False,
                      rgba=None,
                      name='single_contact_device_meshmodel'):
        raise NotImplementedError
