...
class Shuidi(ri.RobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="XYInterface", enable_cc=False):
        ...
        self.jlc.jnts[2].lnk.cmodel = mcm.CollisionModel(
            initor=os.path.join(current_file_dir, "meshes", "shuidi_agv.stl"), cdprim_type=mcm.mc.CDPType.CYLINDER)
        self.jlc.jnts[2].lnk.cmodel.rgba = rm.bc.tab20_list[14]
        self.jlc.finalize()
        # anchor
        ...
        self.anchor.lnk_list[0].cmodel.rgba = rm.bc.tab20_list[14]
        ...
        self.anchor.lnk_list[1].cmodel.rgba = rm.bc.tab20_list[15]
        if enable_cc:
            self.setup_cc()
        # backup
        self.jnt_values_bk = []
