...
class SDA5F(ri.RobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='ur3dual', enable_cc=True):
        ...
        self.lft_body.joints[1]['pos_in_loc_tcp'] = np.array([0.045, 0, 0.7296])
        self.lft_body.joints[2]['pos_in_loc_tcp'] = np.array([0.15, 0.101, 0.1704])
        self.lft_body.joints[2]['gl_rotmat'] = rm.rotmat_from_euler(-math.pi / 2.0, -math.pi / 2.0, 0)
        ...
        self.lft_arm = sia.SIA5(pos=self.lft_body.joints[-1]['gl_posq'],
        ...
        self.rgt_body.joints[1]['pos_in_loc_tcp'] = np.array([0.045, 0, 0.7296])  # right from robot_s view
        self.rgt_body.joints[2]['pos_in_loc_tcp'] = np.array([0.15, -0.101, 0.1704])
        self.rgt_body.joints[2]['gl_rotmat'] = rm.rotmat_from_euler(math.pi / 2.0, -math.pi / 2.0, 0)
        ...
        self.rgt_arm = sia.SIA5(pos=self.rgt_body.joints[-1]['gl_posq'],
