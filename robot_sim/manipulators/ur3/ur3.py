...
class UR3(mi.ManipulatorInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(6), name='ur3', enable_cc=True):
        ...
        self.jlc.jnts[1]['pos_in_loc_tcp'] = np.array([0, 0, .1519])
        self.jlc.jnts[2]['pos_in_loc_tcp'] = np.array([0, 0.1198, 0])
        self.jlc.jnts[2]['gl_rotmat'] = rm.rotmat_from_euler(.0, math.pi / 2.0, .0)
        self.jlc.jnts[2]['loc_motionax'] = np.array([0, 1, 0])
        self.jlc.jnts[3]['pos_in_loc_tcp'] = np.array([0, -.0925, .24365])
        self.jlc.jnts[3]['gl_rotmat'] = rm.rotmat_from_euler(.0, .0, .0)
        self.jlc.jnts[3]['loc_motionax'] = np.array([0, 1, 0])
        self.jlc.jnts[4]['pos_in_loc_tcp'] = np.array([.0, .0, 0.21325])
        self.jlc.jnts[4]['gl_rotmat'] = rm.rotmat_from_euler(.0, math.pi / 2.0, 0)
        self.jlc.jnts[4]['loc_motionax'] = np.array([0, 1, 0])
        self.jlc.jnts[5]['pos_in_loc_tcp'] = np.array([0, .11235 + .0925 - .1198, 0])
        self.jlc.jnts[5]['gl_rotmat'] = rm.rotmat_from_euler(0, 0, 0)
        self.jlc.jnts[5]['loc_motionax'] = np.array([0, 0, 1])
        self.jlc.jnts[6]['pos_in_loc_tcp'] = np.array([0, 0, .08535])
        self.jlc.jnts[6]['gl_rotmat'] = rm.rotmat_from_euler(0, 0, 0)
        self.jlc.jnts[6]['loc_motionax'] = np.array([0, 1, 0])
        self.jlc.jnts[7]['pos_in_loc_tcp'] = np.array([0, .0819, 0])
        self.jlc.jnts[7]['gl_rotmat'] = rm.rotmat_from_euler(-math.pi / 2.0, math.pi / 2.0, 0)
