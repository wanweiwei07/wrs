...
class SIA5(mi.ManipulatorInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(7), name='sia5', enable_cc=True):
        ...
        self.jlc.joints[1]['motion_rng'] = [-math.pi + jnt_safemargin, math.pi]
        self.jlc.joints[2]['pos_in_loc_tcp'] = np.array([0, 0, .168])
        self.jlc.joints[2]['motion_rng'] = [-math.radians(110) + jnt_safemargin, math.radians(110) - jnt_safemargin]
        self.jlc.joints[2]['loc_motionax'] = np.array([0, 1, 0])
        self.jlc.joints[3]['pos_in_loc_tcp'] = np.array([0, 0, .27])
        self.jlc.joints[3]['motion_rng'] = [-math.radians(170) + jnt_safemargin, math.radians(170) - jnt_safemargin]
        self.jlc.joints[4]['pos_in_loc_tcp'] = np.array([.085, 0, 0])
        self.jlc.joints[4]['motion_rng'] = [-math.radians(90) + jnt_safemargin, math.radians(115) - jnt_safemargin]
        self.jlc.joints[4]['loc_motionax'] = np.array([0, -1, 0])
        self.jlc.joints[5]['pos_in_loc_tcp'] = np.array([.27, -0, .06])
        self.jlc.joints[5]['motion_rng'] = [-math.radians(90) + jnt_safemargin, math.radians(90) - jnt_safemargin]
        self.jlc.joints[5]['loc_motionax'] = np.array([-1, 0, 0])
        self.jlc.joints[6]['pos_in_loc_tcp'] = np.array([0, 0, 0])
        self.jlc.joints[6]['motion_rng'] = [-math.radians(110) + jnt_safemargin, math.radians(110) - jnt_safemargin]
        self.jlc.joints[6]['loc_motionax'] = np.array([0, -1, 0])
        self.jlc.joints[7]['pos_in_loc_tcp'] = np.array([.134, .0, .0])
        self.jlc.joints[7]['gl_rotmat'] = rm.rotmat_from_euler(-math.pi / 2, 0, 0)
        self.jlc.joints[7]['motion_rng'] = [-math.radians(90) + jnt_safemargin, math.radians(90) - jnt_safemargin]
        self.jlc.joints[7]['loc_motionax'] = np.array([-1, 0, 0])
        self.jlc.joints[8]['pos_in_loc_tcp'] = np.array([.011, .0, .0])
        self.jlc.joints[8]['gl_rotmat'] = rm.rotmat_from_euler(-math.pi / 2, -math.pi / 2, -math.pi / 2)
        # links
        ...
        self.jlc.lnks[1]['pos_in_loc_tcp'] = np.array([0, 0, .142])
