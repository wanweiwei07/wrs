...
class Robotiq85GelsightPusher(gp.GripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='box', name='robotiq85', enable_cc=True):
        ...
        self.lft_outer.jnts[2]['pos_in_tcp'] = np.array([0, .0315, -.0041])  # passive
        self.lft_outer.jnts[2]['loc_motionax'] = np.array([1, 0, 0])
        self.lft_outer.jnts[3]['pos_in_tcp'] = np.array([0, .0061, .0471])
        self.lft_outer.jnts[3]['loc_motionax'] = np.array([1, 0, 0])
        self.lft_outer.jnts[4]['pos_in_tcp'] = np.zeros(3)
        # https://github.com/Danfoa uses geometry instead of the dae mesh. The following coordiante is needed
        # self.lft_outer.joints[4]['pos_in_tcp'] = np.array([0, -0.0220203446692936, .03242])
        # - lft_inner
        ...
        self.rgt_outer.jnts[2]['pos_in_tcp'] = np.array([0, .0315, -.0041])  # passive
        self.rgt_outer.jnts[2]['loc_motionax'] = np.array([1, 0, 0])
        self.rgt_outer.jnts[3]['pos_in_tcp'] = np.array([0, .0061, .0471])
        self.rgt_outer.jnts[3]['loc_motionax'] = np.array([1, 0, 0])
        self.rgt_outer.jnts[4]['pos_in_tcp'] = np.zeros(3)
        # https://github.com/Danfoa uses geometry instead of the dae mesh. The following coordiante is needed
        # self.rgt_outer.joints[4]['pos_in_tcp'] = np.array([0, -0.0220203446692936, .03242])
        # - rgt_inner
        self.rgt_inner = jl.JLChain(pos=cpl_end_pos, rotmat=cpl_end_rotmat, home_conf=np.zeros(1), name='rgt_inner')
        self.rgt_inner.jnts[1]['pos_in_tcp'] = np.array([0, .0127, .06142])
        ...
        self.middle.jnts[1]['pos_in_tcp'] = np.array([0, 0, .086])
    ...
    #     rotmat = math.asin(math.asin(((jaw_width / 2.0 + 0.0064) - 0.0127) / 0.05715))
        ...
        #     [0.0, 0.0, 0.06325144]) + self.coupling.joints[-1]['pos_in_tcp']
