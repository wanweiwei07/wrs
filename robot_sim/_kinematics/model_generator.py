...
def gen_tcp_frame(jlc,
    ...
    mgm.gen_dashed_stick(spos=jlc.jnts[jlc.tcp_jnt_id].gl_pos_q,
                         epos=tcp_gl_pos,
...
def gen_joint(joint,
    ...
    if joint.type == rkc.JointType.REVOLUTE:
        mgm.gen_stick(spos=spos,
                      ...
                      radius=radius,
                      rgba=bc.joint_parent_rgba).attach_to(m_col)
        mgm.gen_stick(spos=joint._gl_pos_0,
                      epos=epos,
                      radius=radius,
    ...
    if toggle_frame_0:
        mgm.gen_dashed_frame(pos=joint._gl_pos_0,
                             rotmat=joint._gl_rotmat_0,
                             axis_radius=frame_stick_radius,
                             axis_length=frame_stick_length).attach_to(m_col)
    if toggle_frame_q:
        mgm.gen_frame(pos=joint._gl_pos_q,
