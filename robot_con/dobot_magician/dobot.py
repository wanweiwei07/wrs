...
class Dobot:
    ...
    def __init__(self, port, verbose=False):
        ...
        if self.verbose:
            print('dobot_magician: %s open' % self.ser.name if is_open else 'failed to open serial port')
    ...
    def _send_command(self, msg, wait=False):
        ...
        if self.verbose:
            print('dobot_magician: waiting for command', expected_idx)
        ...
        while True:
            ...
            if self.verbose:
                print('dobot_magician: command %d executed' % current_idx)
            break
    ...
    def close(self):
        ...
        self.ser.close()
        if self.verbose:
            print('dobot_magician: %s closed' % self.ser.name)
        self.lock.release()
...
if __name__ == '__main__':
    import numpy as np
    import robot_sim.manipulators.dobot_magician.dobot_magician as dm
    ...
    robot_c = Dobot(port='com4', verbose=True)
    x,y,z,r,j1,j2,j3,j4 = robot_c.pose()
    robot_s.fk(np.array(j1,j2,j3))

    robot_s.gen_meshmodel().attach(base)
    base.run()