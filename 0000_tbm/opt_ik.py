...
class OptIK(object):
    ...
    def __init__(self, rbt_s, component_name, obstacle_list=[], toggle_debug=False):
        self.rbt = rbt_s
        ...
        self.y_err = []
        self.z_err = []
        self._xangle_limit = math.pi/12 # less than .5 degree
        self._yangle_limit = 1e-6
        self._zangle_limit = math.pi/12
        self._x_limit = 1e-6
        self._y_limit = 1e-6
        self._z_limit = 1e-6
    ...
    def _constraint_yangle(self, jnt_values):
        self.rbt.fk(joint_values=jnt_values, component_name=self.jlc_name)
        gl_tcp_pos, gl_tcp_rotmat = self.rbt.get_gl_tcp(manipulator_name=self.jlc_name)
        delta_angle = rm.angle_between_vectors(gl_tcp_rotmat[:,1], self.tgt_rotmat[:,1])
        self.yangle_err.append(delta_angle)
        return self._yangle_limit-delta_angle
